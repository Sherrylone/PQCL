...
class MultiCropWrapper(nn.Module):
    ...
    """
    def __init__(self, backbone, head=None, img_head=False):
    ...
    def teacher_forward(self, global_views, local_views):
        ...
        for i in range(len(global_views)):
            global_outs.append(self.backbone(global_views[i]))
        for i in range(len(local_views)):
            local_outs.append(self.backbone(local_views[i])[:, 1:])
        global_outs = torch.cat(global_outs, dim=0)
        local_outs = torch.cat(local_outs, dim=1)
        return self.head(global_outs), self.head(local_outs)

    def student_forward(self, global_views, local_pos, mask=None, **kwargs):
        if not isinstance(global_views, list):
            global_views = [global_views]
            local_pos = [local_pos]
        ...
        local_outs = []
        for i in range(len(global_views)):
            global_out, local_out = self.backbone(global_views[i], local_pos[i], mask[i])
            global_outs.append(global_out)
            local_outs.append(local_out)
        global_outs = torch.cat(global_outs, dim=0)
        local_outs = torch.cat(local_outs, dim=0)
        global_outs = self.head(global_outs)
        local_outs = self.head(local_outs)
        return global_outs, local_outs

    def forward(self, global_views, local_param=None, mask=None, **kwargs):
        if mask is not None:
            return self.student_forward(global_views, local_param, mask)
        else:
            return self.teacher_forward(global_views, local_param)
